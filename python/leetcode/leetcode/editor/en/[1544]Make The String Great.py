...
#  
# 
#  Related Topics String Stack 👍 2609 👎 133

import unittest


class MakeTheStringGreat_Test(unittest.TestCase):

    def test_makeTheStringGreat(self):
        test_data = [
            ("abBAcC", ""),
            ("leEeetcode", "leetcode")
        ]
        for s, expected in test_data:
            with self.subTest(s=s, expected=expected):
                self.assertEqual(expected, Solution().makeGood(s))
