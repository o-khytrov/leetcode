...
#  
# 
#  Related Topics Array Two Pointers Simulation 👍 2679 👎 129

import unittest
from typing import List


class RearrangeArrayElementsBySign_Test(unittest.TestCase):

    def test_rearrangeArrayElementsBySign(self):
        test_data = [([3, 1, -2, -5, 2, -4], [3, -2, 1, -5, 2, -4]),
                     ([-1, 1], [1, -1])
                     ]
        for arr, expected in test_data:
            with self.subTest(arr=arr, expected=expected):
                self.assertSequenceEqual(expected, Solution().rearrangeArray(arr))
