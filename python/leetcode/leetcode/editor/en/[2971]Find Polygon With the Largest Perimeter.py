...
# is not possible to include 2 or more smaller sides that have a greater sum than 
# either of them.
...
#  
# 
#  Related Topics Array Greedy Sorting Prefix Sum 👍 397 👎 42

import unittest
from typing import List


class FindPolygonWithTheLargestPerimeter_Test(unittest.TestCase):

    def test_findPolygonWithTheLargestPerimeter(self):
        test_data = [
            ([5, 5, 5], 15),
            ([5, 5, 50], -1),
            ([1, 12, 1, 2, 5, 50, 3], 12)
        ...
        for nums, expected in test_data:
            with self.subTest(nums=nums, expected=expected):
                self.assertEqual(expected, Solution().largestPerimeter(nums))
