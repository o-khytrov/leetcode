...
#  
# 
#  Related Topics Array Queue Simulation 👍 1102 👎 92

import unittest
from typing import List


class TimeNeededToBuyTickets_Test(unittest.TestCase):

    def test_timeNeededToBuyTickets(self):
        test_data = [
            ([2, 3, 2], 2, 6),
            ([5, 1, 1, 1], 0, 8)
        ]
        for tickets, k, expected in test_data:
            with self.subTest(tickets=tickets, k=k):
                self.assertEqual(expected, Solution().timeRequiredToBuy(tickets, k))
...
class Solution:
    def timeRequiredToBuy(self, tickets: List[int], k: int) -> int:
        ...
        while queue:
            ...
            time += 1
            if current[0] == k and current[1] == 0:
                return time
