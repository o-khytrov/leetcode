...
// 
//
// Related Topics Array Two Pointers Greedy Sorting 👍 3063 👎 283
...
namespace AssignCookies
{
    public class Tests
    {
        ...
        public void AssignCookiesTest(string gJson, string sJson, int expectedResult)
        {
            var g = JsonSerializer.Deserialize<int[]>(gJson)
                    ?? throw new ArgumentException("Invalid Json");

            var s = JsonSerializer.Deserialize<int[]>(sJson)
                    ?? throw new ArgumentException("Invalid Json");
            Assert.Equal(expectedResult, new Solution().FindContentChildren(g, s));
        }
    }
