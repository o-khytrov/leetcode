...
row (notice that, there are two cells with value 3 and we can remove any of them)
...
The final answer = 10.
 ...
 Related Topics Array Sorting Heap (Priority Queue) Matrix Simulation 👍 544 👎
33
...
namespace DeleteGreatestValueInEachRow
{
    ...
    {
        [Theory]
        [InlineData("[[1,2,4],[3,3,1]]", 8)]
        [InlineData("[[10]]", 10)]
        public void DeleteGreatestValueInEachRowTest(string matrixJson, int expectedResult)
        {
            var matrix = JsonSerializer.Deserialize<int[][]>(matrixJson)
                         ?? throw new ArgumentException("Invalid Json");
            Assert.Equal(expectedResult, new Solution().DeleteGreatestValue(matrix));
        }
    }
