...
in the resulting list.
 ...
 Related Topics Array Hash Table Two Pointers 👍 272 👎 9
...
namespace MergeTwo2DArraysBySummingValues
{
    ...
    {
        [Theory]
        [InlineData("[[1,2],[2,3],[4,5]]", "[[1,4],[3,2],[4,1]]", "[[1,6],[2,3],[3,2],[4,6]]")]
        public void MergeTwo2DArraysBySummingValuesTest(string nums1Json, string nums2Json, string expectedResultJson)
        {
            var nums1 = JsonSerializer.Deserialize<int[][]>(nums1Json) ?? throw new ArgumentException("Invalid Json");
            var nums2 = JsonSerializer.Deserialize<int[][]>(nums2Json) ?? throw new ArgumentException("Invalid Json");
            var result = new Solution().MergeArrays(nums1, nums2);
...
//leetcode submit region begin(Prohibit modification and deletion)
    public class Solution
    {
        public int[][] MergeArrays(int[][] nums1, int[][] nums2)
        {
            ...
            return map.OrderBy(x => x.Key).Select(x => new int[] { x.Key, x.Value }).ToArray();
        }
    }
