...
Hence, we return false.
 ...
 Related Topics Array Hash Table Matrix 👍 904 👎 48
...
namespace CheckIfEveryRowAndColumnContainsAllNumbers
{
    ...
    {
        [Theory]
        [InlineData("[[1,2,3],[3,1,2],[2,3,1]]", true)]
        [InlineData("[[1,1,1],[1,2,3],[1,2,3]]", false)]
        [InlineData(
            ...
            false)]
        public void CheckIfEveryRowAndColumnContainsAllNumbersTest(string matrixJson, bool expectedResult)
        {
            var matrix = JsonSerializer.Deserialize<int[][]>(matrixJson)
                         ?? throw new ArgumentException("Invalid json");
            Assert.Equal(expectedResult, new Solution().CheckValid(matrix));
        }
    }
...
//leetcode submit region begin(Prohibit modification and deletion)
    public class Solution
    {
        public bool CheckValid(int[][] matrix)
        {
            ...
            var colMap = new HashSet<int>[matrix.Length];
            ...
            for (var i = 0; i < matrix.Length; i++)
            {
                ...
                for (int j = 0; j < matrix[i].Length; j++)
                {
                    ...
                    colMap[j].Add(matrix[i][j]);
                }

                if (rowMap.Count != matrix[i].Length)
                {
                    return false;
                }
            }
            ...
            {
                if (colMap[i].Count != matrix[i].Length)
                {
                    return false;
                }
            }
