...
".
 ...
 Related Topics String Simulation 👍 497 👎 38
...
namespace CalculateDigitSumOfAString
{
    ...
    {
        [Theory]
        [InlineData("11111222223", 3, "135")]
        [InlineData("00000000", 3, "000")]
        public void CalculateDigitSumOfAStringTest(string s, int k, string expectedResult)
        {
            Assert.Equal(expectedResult, new Solution().DigitSum(s, k));
        }
    }
...
//leetcode submit region begin(Prohibit modification and deletion)
    public class Solution
    {
        public string DigitSum(string s, int k)
        {
            ...
            var digitSum = 0;
            while (tmpStr.Length > k)
            {
                for (var i = 0; i < tmpStr.Length; i++)
                {
                    digitSum += tmpStr[i] - 48;
                    if ((i + 1) % k == 0 || i == tmpStr.Length - 1)
                    {
