...
the image.
 ...
 Related Topics Array Depth-First Search Breadth-First Search Matrix 👍 7885 👎
776
...
namespace FloodFill
{
    public class Tests
    {
        ...
        public void FloodFillTest(string imageJson, int sr, int sc, int color, string expectedResultJson)
        {
            var image = JsonSerializer.Deserialize<int[][]>(imageJson)
                        ?? throw new ArgumentException("Invalid Json");

            Assert.Equal(expectedResultJson, JsonSerializer.Serialize(new Solution().FloodFill(image, sr, sc, color)));
        }
    }
...
//leetcode submit region begin(Prohibit modification and deletion)
    ...
    {
        public void FloodFillRecursively(int[][] image, int sr, int sc, int color, int sourceColor)
        {
            ...
            image[sr][sc] = color;
            FloodFillRecursively(image, sr + 1, sc, color, sourceColor);
            FloodFillRecursively(image, sr - 1, sc, color, sourceColor);
            FloodFillRecursively(image, sr, sc + 1, color, sourceColor);
            FloodFillRecursively(image, sr, sc - 1, color, sourceColor);
        }
        ...
        public int[][] FloodFill(int[][] image, int sr, int sc, int color)
        {
            ...
            FloodFillRecursively(image, sr, sc, color, sourceColor);
            return image;
        }
    }
