...
// 
//
// Related Topics Array Two Pointers 👍 6211 👎 1175
...
namespace RemoveDuplicatesFromSortedArrayII
{
    ...
    {
        [Theory]
        [InlineData("[1,1,1,2,2,3]", 5, "[1,1,2,2,3,0]")]
        [InlineData("[0,0,1,1,1,1,2,3,3]", 7, "[0,0,1,1,2,3,3,0,0]")]
        public void RemoveDuplicatesFromSortedArrayIITest(string numsJson, int expectedResult, string expectedArrayJson)
        {
            ...
            Assert.True(JsonSerializer.Deserialize<int[]>(expectedArrayJson).Take(result)
                .SequenceEqual(nums.Take(result)));
        }
    }
