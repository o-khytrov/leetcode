...
// 
//
// Related Topics Array Sorting 👍 867 👎 39
...
namespace ReductionOperationsToMakeTheArrayElementsEqual
{
    public class Tests
    {
        ...
        public void ReductionOperationsToMakeTheArrayElementsEqualTest(string numsJson, int expectedResult)
        {
            var nums = JsonSerializer.Deserialize<int[]>(numsJson)
                       ?? throw new ArgumentException("Invalid json");
            Assert.Equal(expectedResult, new Solution().ReductionOperations(nums));
        }
    }
...
//leetcode submit region begin(Prohibit modification and deletion)
    public class Solution
    {
        public int ReductionOperations(int[] nums)
        {
            ...
            {
                if (map.TryGetValue(x, out var value))
                {
                    sum += value;
                    count += sum;
                }
