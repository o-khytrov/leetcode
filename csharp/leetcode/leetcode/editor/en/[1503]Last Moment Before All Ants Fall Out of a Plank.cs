...
//plank, each ant moves with a speed of 1 unit per second. Some of the ants move to 
//the left, the other move to the right. 
//
// When two ants moving in two different directions meet at some point, they 
//change their directions and continue moving again. Assume changing directions does 
...
//the ants moving to the left and the right, return the moment when the last ant(s) 
...
// 
//
// Related Topics Array Brainteaser Simulation 👍 1408 👎 374
...
namespace LastMomentBeforeAllAntsFallOutOfAPlank
{
    ...
    {
        [Theory]
        [InlineData(4, "[4,3]", "[4,3]", 4)]
        [InlineData(4, "[4,3]", "[0,1]", 4)]
        [InlineData(7, "[0,1,2,3,4,5,6,7]", "[]", 7)]
        public void LastMomentBeforeAllAntsFallOutOfAPlankTest(int n, string leftJson, string rightJson,
            int expectedResult)
        {
            var right = JsonSerializer.Deserialize<int[]>(rightJson)
                        ?? throw new ArgumentException("Invalid json");

            var left = JsonSerializer.Deserialize<int[]>(leftJson)
                       ?? throw new ArgumentException("Invalid json");
            Assert.Equal(expectedResult, new Solution().GetLastMoment(n, left, right));
        }
    }
