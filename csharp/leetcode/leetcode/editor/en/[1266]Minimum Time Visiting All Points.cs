...
// 
//
// Related Topics Array Math Geometry 👍 1770 👎 195
...
namespace MinimumTimeVisitingAllPoints
{
    ...
    {
        [Theory]
        [InlineData("[[1,1],[3,4],[-1,0]]", 7)]
        [InlineData("[[3,2],[-2,2]]", 5)]
        public void MinimumTimeVisitingAllPointsTest(string pointsJson, int expectedResult)
        {
            var points = Helper.ToIntMatrix(pointsJson);
            Assert.Equal(expectedResult, new Solution().MinTimeToVisitAllPoints(points));
        }
    }
    ...
    public class Solution
    {
        public int MinTimeToVisitAllPoints(int[][] points)
        {
            ...
            for (var p = 1; p < points.Length; p++)
            {
                ...
                {
                    count += horizontalDistance;
                }
                else
                {
                    var min = Math.Min(horizontalDistance, verticalDistance);
                    var max = Math.Max(horizontalDistance, verticalDistance);
                    count += min;
                    count += max - min;
                }
