...
// 
//
// Related Topics Array Math Greedy Sorting Game Theory 👍 1421 👎 154
...
namespace MaximumNumberOfCoinsYouCanGet
{
    ...
    {
        [Theory]
        [InlineData("[2,4,1,2,7,8]", 9)]
        [InlineData("[2,4,5]", 4)]
        [InlineData("[9,8,7,6,5,1,2,3,4]", 18)]
        public void MaximumNumberOfCoinsYouCanGetTest(string pilesJson, int expectedResult)
        {
            var piles = JsonSerializer.Deserialize<int[]>(pilesJson)
                        ?? throw new ArgumentException("Invalid json");
            Assert.Equal(expectedResult, new Solution().MaxCoins(piles));
        }
    }
...
//leetcode submit region begin(Prohibit modification and deletion)
    public class Solution
    {
        ...
        {
            var count = 0;
            var sorted = piles.OrderByDescending(x => x).ToList();
            while (sorted.Count > 0)
            {
                sorted.RemoveAt(0); // alice
                count += sorted[0];
                sorted.RemoveAt(0);
