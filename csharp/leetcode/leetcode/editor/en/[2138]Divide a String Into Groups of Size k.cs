...
group consists of the next k characters of the string, and so on. Each character
...
Thus, the 4 groups formed are "abc", "def", "ghi", and "jxx".
 ...
 Related Topics String Simulation 👍 424 👎 17
...
namespace DivideAStringIntoGroupsOfSizeK
{
    ...
    {
        [Theory]
        [InlineData("abcdefghi", 3, 'x', "[\"abc\",\"def\",\"ghi\"]")]
        [InlineData("abcdefghij", 3, 'x', "[\"abc\",\"def\",\"ghi\",\"jxx\"]")]
        public void DivideAStringIntoGroupsOfSizeKTest(string s, int k, char fill, string expectedResultJson)
        {
            var sut = new Solution();
            var result = sut.DivideString(s, k, fill);
...
//leetcode submit region begin(Prohibit modification and deletion)
    public class Solution
    {
        public string[] DivideString(string s, int k, char fill)
        {
            ...
            {
                var length = Math.Min(k, s.Length - i);
                var slice = span.Slice(i, length).ToString().PadRight(k, fill);
                result.Add(slice);
            }
